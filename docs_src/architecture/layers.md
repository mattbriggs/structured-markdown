...
**Classifier** (`classifier.py`): Splits the `RawNode` list at H2 heading boundaries, treating each H2 and the nodes that follow it as a `Unit`. The classifier infers the `UnitType` from the heading title text using keyword matching and can infer procedure-like units from ordered lists or code blocks when headings are not specific. The overall article type is selected from document-level metadata keys such as `articleType`, `article_type`, or `type` when a recognized value is present. If metadata is absent or unsupported, the classifier scores the unit-type population against runtime article signatures and chooses the closest known article type, falls back to `topic` for mixed known units, or emits SP-041 when no article type can be determined.
