...
- The article type is read from front matter (`articleType`, `article_type`, or `type` key) and mapped against `_ARTICLE_TYPE_MAP`. If no match is found, `ArticleType.unknown` is assigned and an SP-041 diagnostic is emitted.

This approach is not general-purpose. It is purpose-built to map Markdown to a specific schema and produces useful results only for content that follows the conventions the schema expects. A Markdown document that uses heading levels unconventionally, lacks front matter, or organizes content in ways the classifier does not recognize will receive `unknown` triage status on the unclassified units and corresponding SP-040 diagnostics — but the parse will still complete and produce a partial structured result.
